...
class Student extends CI_Controller { 
    ...
    if ($this->session->userdata('admin_login') == 1) redirect(base_url() . 'student/dashboard', 'refresh');
    }
      /************* / default functin, redirects to login page if no admin logged in yet***/
    ...
    function teacher (){
        ...
        $student_profile = $this->db->get_where('student', array('student_id' => $this->session->userdata('student_id')))->row();
         ...
         $subject_class_id = $this->db->get_where('subject', array('class_id' => $select_student_class_id))->row()->teacher_id;

        $page_data['page_name']     = 'teacher';
        $page_data['page_title']    = get_phrase('Class Teachers');
        $page_data['student_teacher'] = $this->db->get_where('teacher', array('teacher_id' => $subject_class_id))->result_array();
        $this->load->view('admin/index', $page_data);
    }
    ...
    public function studentBookRequest($param1 = null, $param2 = null, $param3 = null){

        if($param1 == 'insert'){
            $this->student_model->createStudentBookRequest();
            $this->session->set_flashdata('flash_message', get_phrase('Data Onserted successfully'));
            redirect(base_url() . 'student/studentBookRequest', 'refresh');
        }

        $page_data['page_name']     = 'student_book_request';
        $page_data['page_title']    = get_phrase('Book Request');
        $this->load->view('admin/index', $page_data);
