<div class="row">
 ...
 <?php echo form_open(base_url().'payment/paymentSetting/update', array('class' => 'form-horizontal form-groups-bordered validate', 'enctype'=>'multipart/form-data'));?>
            	<strong>STRIP PAYMENT SETTINGS</strong>&nbsp;<i class="fa fa-angle-double-right"></i> <a href="https://dashboard.stripe.com/register">Register Here</a>
				  <span class="pull-right"> <input type="checkbox" name="stripe"  class="js-switch" id="notify" value="yes" <?php if($this->db->get_where('settings' , array('type' =>'stripe'))->row()->description == 'yes') echo 'checked';?>  data-color="#e625ea" /></span>
				<hr>

								<?php
                                $stripe_settings = $this->crud_model->get_settings('stripe_keys');
				...
				<span class="pull-right"> <input type="checkbox" name="paypal_setting"  class="js-switch" id="notify" value="yes" <?php if($this->db->get_where('settings' , array('type' =>'paypal_setting'))->row()->description == 'yes') echo 'checked';?> data-color="#e625ea" /></span>
				<hr>
								 <?php
                                $paypal_settings = $this->crud_model->get_settings('paypal');
